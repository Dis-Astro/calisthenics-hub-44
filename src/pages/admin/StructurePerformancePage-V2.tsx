...
const StructurePerformancePageV2 = () => {
  ...
  const getDateRange = () => {
    ...
    let start, end, dataPoints: number;
    ...
    switch (period) {
      ...
      case "custom":
        ...
        dataPoints = Math.ceil((end.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) + 1;
        break;
      default:
        start = startOfMonth(today);
        end = endOfMonth(today);
    ...
    return { start: startOfDay(start), end: endOfDay(end), dataPoints };
  };

  const fetchData = async () => {
    setLoading(true);
    const { start, end, dataPoints } = getDateRange();
    const startStr = format(start, "yyyy-MM-dd");
    const endStr = format(end, "yyyy-MM-dd");

    try {
      // Fetch payments (revenues)
      ...
      const metrics: BusinessLineMetrics[] = [
        {
          name: "Palestra",
          revenue: (payments || []).filter(p => p.service_type === "palestra").reduce((sum, p) => sum + (p.amount || 0), 0),
          costs: (expenses || []).filter(e => e.subcategory === "Palestra").reduce((sum, e) => sum + (e.amount || 0), 0),
          margin: 0,
        ...
        {
          name: "Coaching",
          revenue: (payments || []).filter(p => p.service_type === "coaching").reduce((sum, p) => sum + (p.amount || 0), 0),
          costs: (expenses || []).filter(e => e.subcategory === "Coaching").reduce((sum, e) => sum + (e.amount || 0), 0),
          margin: 0,
        ...
        {
          name: "Corsi",
          revenue: (payments || []).filter(p => p.service_type === "corso").reduce((sum, p) => sum + (p.amount || 0), 0),
          costs: (expenses || []).filter(e => e.subcategory === "Corsi").reduce((sum, e) => sum + (e.amount || 0), 0),
          margin: 0,
...
export default StructurePerformancePageV2;
